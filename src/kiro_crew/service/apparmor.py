...
def install(
    ...
    scope = (
        " — grants unprivileged userns to processes executed through the "
        "attached kirocrew launcher path (the service's ExecStart, and any "
        "launch that resolves to the same file), and enforcement is confirmed"
        if resolved_exec is not None
        else " — a named profile with no attachment; nothing runs under it "
        "until something applies it explicitly. Enforcement is confirmed"
    )
    return ProfileOutcome(
        True,
