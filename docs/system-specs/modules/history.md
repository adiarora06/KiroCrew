...
- `_read_messages(key)` — mtime-cached JSONL parse, holding the session's
  in-process writer lock (`_file_lock`, not the heavier cross-process
  `_locked`) across the whole stat/read/cache-store sequence. Without it, a
  mtime-preserving `rewrite_session` (compaction, rewind) landing inside that
  window could publish a cache entry holding pre-rewrite content under the
  restored mtime — undetectable by any later mtime comparison, so every
  reader kept getting stale messages until process restart.
